...
class MLTDThread(threading.Thread, metaclass=abc.ABCMeta):
    ...
    def run(self):
        ...
        while not self._should_quit():
            ...
            if not can_loop:
                self.stop()
        self._cleanup()
        MLTDThreadList().remove(self)
        LOG.debug("Exiting")


class MLTDThreadList:
    """Singleton class that keeps track of application wide threads."""

    _instance = None

    threads_list: List[MLTDThread] = []
    lock = threading.Lock()
