...
import mltd
from mltd import conf  # noqa: F401
...
DEFAULT_SAVE_FILE = f"{home}/.config/mltd/mltd.p"
DEFAULT_CONFIG_FILE = f"{home}/.config/mltd/mltd.conf"
...
def process_standard_options(f: F) -> F:
    def new_func(*args, **kwargs):
        ...
        try:
            CONF(
                [], project="mltd", version=mltd.__version__,
        ...
        if not config_file_found:
            LOG = logging.getLogger("MLTD")   # noqa: N806
            LOG.error("No config file found!! run 'mltd sample-config'")
